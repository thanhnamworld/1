        ...
        <div className="absolute w-full flex justify-between px-2">
          {days.map((day, i) => (
            <div key={i} className="flex flex-col items-center">
              ...
              <span className="text-[8px] font-black text-slate-300 mt-1 uppercase tracking-tighter">{day}</span>
            </div>
          ))}
        </div>
        <div className="animated-car flex items-center gap-1">
          <div className="p-1 bg-emerald-500 rounded-lg shadow-lg shadow-emerald-100">
            <Car size={10} className="text-white" />
          </div>
          <div className="w-4 h-[1px] bg-emerald-300 border-t border-dashed border-emerald-400 opacity-40"></div>
        </div>
      </div>
    </div>
  );
...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, notifications, clearNotification, profile, onLoginClick }) => {
  ...
  const manageItems = [
    ...
    { id: 'manage-trips', label: 'QL Chuyến Xe', icon: ClipboardList, roles: ['admin', 'manager', 'driver'] },
    { id: 'manage-orders', label: 'QL Đơn Hàng', icon: ShoppingBag, roles: ['admin', 'manager', 'driver'] },
    { id: 'admin', label: 'QL Hệ thống', icon: Shield, roles: ['admin'] },
  ];
    ...
    <div className="flex h-screen bg-[#F8FAFC] overflow-hidden flex-col lg:flex-row">
      ...
      <aside className="hidden lg:flex flex-col w-72 bg-white border-r border-slate-100 p-8 shrink-0">
        <div className="flex items-center gap-3 mb-10 px-2">
          ...
          <span className="text-xl font-extrabold tracking-tight text-slate-900 italic">
          ...
          {profile ? (
            <div className="bg-slate-50 border border-slate-100 p-4 rounded-3xl space-y-3">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-white flex items-center justify-center text-emerald-600 font-black text-base shadow-sm border border-slate-100">
                  {profile.full_name?.charAt(0) || 'U'}
                </div>
                <div className="min-w-0 text-left">
                  <p className="text-xs font-black text-slate-800 truncate leading-tight">{profile.full_name}</p>
                  <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest mt-1">{profile.role}</p>
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => setActiveTab('profile')} className="flex-1 py-2 bg-white text-slate-400 rounded-xl hover:text-emerald-600 border border-slate-100 flex items-center justify-center gap-2"><Settings size={12} /><span className="text-[9px] font-black uppercase">Hồ sơ</span></button>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-100 flex items-center justify-between px-4 sm:px-8 sticky top-0 z-20">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg sm:text-xl font-extrabold text-slate-900 tracking-tight truncate max-w-[180px]">
              {activeTab === 'admin' ? 'Hệ thống' : 
               activeTab === 'manage-orders' ? 'Đơn hàng' :
               activeTab === 'manage-trips' ? 'Chuyến xe' : 
               activeTab === 'profile' ? 'Tài khoản' : 
               activeTab === 'dashboard' ? 'Thống kê' :
               activeTab === 'post' ? 'Đăng chuyến' :
               activeTab === 'bookings' ? 'Hành trình' : 'Chung đường'}
            </h2>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setShowNotifications(!showNotifications)} className="p-2 sm:p-2.5 text-slate-400 hover:text-emerald-600 hover:bg-slate-50 rounded-xl transition-all relative">
              <Bell size={20} />
              {notifications.filter(n => !n.read).length > 0 && <span className="absolute top-1.5 right-1.5 w-4 h-4 bg-rose-500 text-white text-[8px] font-black flex items-center justify-center rounded-full border-2 border-white">{notifications.filter(n => !n.read).length}</span>}
            </button>
            ...
            {profile ? (
              <button onClick={() => setActiveTab('profile')} className="flex items-center gap-2 p-1.5 bg-slate-50 border border-slate-100 rounded-xl">
                <div className="h-7 w-7 rounded-lg bg-emerald-600 flex items-center justify-center text-white text-[10px] font-black">{profile.full_name?.charAt(0) || 'U'}</div>
                <div className="hidden sm:block text-left"><p className="text-[11px] font-black text-slate-800 leading-none">{profile.full_name}</p></div>
              </button>
            ) : (
              <button onClick={onLoginClick} className="p-2 text-emerald-600 bg-emerald-50 rounded-xl"><LogIn size={20} /></button>
            )}
          </div>
        </header>
          ...
          <div className="lg:hidden fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[60] animate-in fade-in duration-300" onClick={() => setShowManageMenu(false)}>
            <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-[40px] p-8 pb-32 animate-in slide-in-from-bottom-10 duration-500 shadow-2xl" onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-8">
                <div>
                   <h3 className="text-xl font-black text-slate-900 uppercase italic">Trung tâm Quản lý</h3>
                   <p className="text-slate-400 text-xs font-medium">Lựa chọn công cụ điều hành</p>
                </div>
        ...
        <nav className="lg:hidden fixed bottom-6 left-4 right-4 z-[70]">
          <div className="bg-white/90 backdrop-blur-xl border border-white/40 shadow-2xl rounded-[32px] px-2 py-2 flex items-center justify-around">
            <button onClick={() => setActiveTab('search')} className={`flex flex-col items-center gap-1 p-3 flex-1 transition-all ${activeTab === 'search' ? 'text-emerald-600' : 'text-slate-400'}`}>
              ...
              <span className="text-[9px] font-black uppercase tracking-widest">Tìm kiếm</span>
            ...
            <button onClick={() => setActiveTab('bookings')} className={`flex flex-col items-center gap-1 p-3 flex-1 transition-all ${activeTab === 'bookings' ? 'text-emerald-600' : 'text-slate-400'}`}>
              ...
              <span className="text-[9px] font-black uppercase tracking-widest">Lịch trình</span>
